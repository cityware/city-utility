...
class UdpPortScanner {
    ...
    function __construct($hostIp, $startPort = 1, $endPort = 1024, $timeout = 1) {
    ...
    private function scanPort($portNumber) {
        ...
        $errno = $errstr = null;
        $handle = fsockopen($this->hostIp, $portNumber, &$errno, &$errstr, 2);
        if (!$handle) {
    ...
    private function networkProbe($noTrials = 100, $startPortNumber = 55000) {
        ...
        $this->cleanupIterations = $this->calculateNoIterations($estFalsePositives, $percentResponses, $portRange);
    }
    ...
    private function getMicroTime() {
        return ((double) strstr(microtime(), ' ') + (double) substr(microtime(), 0, strpos(microtime(), ' ')));
    }
